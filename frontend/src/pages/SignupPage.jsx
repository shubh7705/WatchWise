...
import { Film, Lock, User, Mail, Sparkles } from 'lucide-react';
import { useAuth } from '../context/AuthContext';
import { useMovies } from '../context/MovieContext';

export const SignupPage = ({ onNavigateToLogin, onSuccess }) => {
  ...
  const handleSubmit = (e) => {
    ...
    const result = signup({ username, email, password, bio });
    if (result.success) {
      showToast(`Welcome to WatchWise, ${username}! 🎉`);
      onSuccess();
    } else {
    ...
    <div className="app-container" style={{ padding: '60px 0 80px', display: 'flex', justifyContent: 'center' }}>
      ...
      >
        <div style={{ textAlign: 'center', marginBottom: '28px' }}>
          ...
          <h2 style={{ fontSize: '1.6rem', fontWeight: 900 }}>Create an Account</h2>
          <p style={{ color: 'var(--text-secondary)', fontSize: '0.88rem' }}>
            Join thousands of cinema lovers sharing reviews and building watchlists.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ marginBottom: '24px' }}>
          ...
          <div style={{ marginBottom: '22px' }}>
            ...
            <input
              type="text"
              placeholder="What types of cinema do you enjoy?"
              value={bio}
